...
namespace API.Helpers
{
    public class AutoMaping : Profile
    {
        ...
        CreateMap<Menu, MenuDto>()
         .ForMember(vr => vr.SubDto, opt => opt.MapFrom(v => v.SubMenu.Select(vf => new SubDto {id = vf.Id, Name = vf.Name, image = vf.image, SubSubDto = vf.SubSubMenu.Select( s => new SubSubDto{id = s.Id, Name = s.Name, image = s.image})})));
